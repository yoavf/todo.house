...
from app.models import LocationCreate, LocationUpdate, LocationType
...
class TestLocationEndpoints:
    ...
    async def test_create_location_success(self, client, test_user_id):
        ...
        assert data["metadata"] == {"floor": 1, "size": "large"}
        assert "id" in data
    ...
    async def test_update_location(self, client, test_user_id):
        ...
        assert data["metadata"] == {"has_workbench": True}
    ...
    async def test_delete_location_soft_delete(self, client, test_user_id):
        ...
        deleted_location = next((loc for loc in locations if loc["id"] == location_id), None)
        if deleted_location:
