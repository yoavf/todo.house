...
class PromptTestingCLI:
    ...
    async def compare_prompts(
        self, scenario: str, custom_prompts: Optional[List[str]] = None
    ):
        ...
        try:
            ...
            print("\n📊 Comparison Results:")
            print(
                f"Best prompt: {prompt_names[comparison.best_prompt_index]} (Accuracy: {comparison.best_accuracy:.3f})"
            )
            ...
            for i, result in enumerate(comparison.results):
                ...
                print(
                    f"{i + 1}. {prompt_names[i]}: {status_icon} {result.accuracy_score:.3f} accuracy, {result.processing_time:.2f}s"
                )
                if result.error_message:
                    print(f"   Error: {result.error_message}")
            ...
            print(
                f"Matched: {len(best_result.matched_tasks)}, Missing: {len(best_result.missing_tasks)}, Unexpected: {len(best_result.unexpected_tasks)}"
            )
    ...
    async def batch_test(self, prompt: str, scenarios: Optional[List[str]] = None):
        ...
        try:
            ...
            print(
                f"Average processing time: {report['summary']['average_processing_time']:.2f}s"
            )
            print(
                f"Total tasks generated: {report['summary']['total_tasks_generated']}"
            )
            ...
            for scenario, stats in report["scenario_breakdown"].items():
                success_rate = (
                    stats["successful"] / stats["tests"] if stats["tests"] > 0 else 0
                )
                print(
                    f"{scenario}: {success_rate:.1%} success, {stats['avg_accuracy']:.3f} accuracy, {stats['avg_tasks']:.1f} tasks"
                )

            # Show best and worst performing scenarios
            scenario_accuracies = [
                (s, stats["avg_accuracy"])
...
Examples:
  ...
  %(prog)s generate-report --format json
    ...
    compare_parser = subparsers.add_parser(
        "compare-prompts", help="Compare multiple prompts"
    )
    compare_parser.add_argument("--scenario", required=True, help="Test scenario")
    compare_parser.add_argument(
        "--prompts", nargs="+", help="Custom prompts to compare"
    )

    # Batch test
    batch_parser = subparsers.add_parser(
        "batch-test", help="Run batch test across scenarios"
    )
    batch_parser.add_argument("--prompt", required=True, help="Prompt to test")
    batch_parser.add_argument(
        "--scenarios", nargs="+", help="Specific scenarios to test"
    )
    ...
    report_parser = subparsers.add_parser(
        "generate-report", help="Generate test report"
    )
    ...
    image_parser = subparsers.add_parser(
        "create-image", help="Create custom test image"
    )
