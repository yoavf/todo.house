...
async def test_populate_task_image_urls_no_images():
    ...
    tasks = [
        MockTaskModel(id=1, title="Task 1"),
        MockTaskModel(id=2, title="Task 2")
    ]
...
async def test_populate_task_image_urls_with_images():
    ...
    with patch('app.tasks.storage') as mock_storage:
        mock_storage.get_public_url.side_effect = lambda path: f"https://storage.example.com/{path}"
        
        result = await populate_task_image_urls(tasks, session)
    
    # Verify results
    assert len(result) == 3
    assert result[0].image_url == "https://storage.example.com/path/to/image1.jpg"
    assert result[0].thumbnail_url == "https://storage.example.com/path/to/image1.jpg"
    assert result[1].image_url == "https://storage.example.com/path/to/image2.jpg"
    assert result[1].thumbnail_url == "https://storage.example.com/path/to/image2.jpg"
    assert result[2].image_url is None
    assert result[2].thumbnail_url is None
    
    # Verify storage was called only twice (caching works)
    assert mock_storage.get_public_url.call_count == 2
...
async def test_populate_task_image_urls_database_error():
    ...
    image_id = uuid.uuid4()
    tasks = [
        MockTaskModel(id=1, title="Task 1", source_image_id=image_id)
    ]
...
async def test_populate_task_image_urls_storage_error():
    ...
    with patch('app.tasks.storage') as mock_storage:
        mock_storage.get_public_url.side_effect = Exception("Storage service unavailable")
        
        with patch('app.tasks.logger') as mock_logger:
            result = await populate_task_image_urls(tasks, session)
    
    # Should return task without URLs
    assert len(result) == 1
    assert result[0].image_url is None
    assert result[0].thumbnail_url is None
    ...
    assert "Failed to generate storage URL for image" in warning_call[0][0]
    assert warning_call[1]["error_type"] == "Exception"
...
async def test_populate_task_image_urls_caching():
    ...
    # Mock storage
    with patch('app.tasks.storage') as mock_storage:
        mock_storage.get_public_url.return_value = f"https://storage.example.com/{shared_path}"
        
        result = await populate_task_image_urls(tasks, session)
    
    # Verify both tasks have the same URL
    assert result[0].image_url == f"https://storage.example.com/{shared_path}"
    assert result[1].image_url == f"https://storage.example.com/{shared_path}"
    
    # Verify storage was called only once (caching prevented duplicate call)
    mock_storage.get_public_url.assert_called_once_with(shared_path)