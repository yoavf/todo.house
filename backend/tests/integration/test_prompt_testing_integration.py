...
class TestPromptTestingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_prompt_testing_with_mock_provider(self, test_library, prompt_library):
        ...
        with patch('app.ai.providers.GeminiProvider') as MockProvider:
            ...
            assert "limescale" in result.matched_tasks[0].lower() or "soap" in result.matched_tasks[0].lower()
            ...
            assert mock_instance.analyze_image.call_count >= 3  # 1 single + 2 comparison
    ...
    async def test_batch_testing_workflow(self, test_library, prompt_library):
        ...
        with patch('app.ai.providers.GeminiProvider') as MockProvider:
            ...
            test_scenarios = [TestScenario.BATHROOM_SINK, TestScenario.KITCHEN_APPLIANCES]
