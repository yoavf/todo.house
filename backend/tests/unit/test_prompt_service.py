...
from app.ai.prompt_service import (
    PromptService,
    PromptNotFoundError,
    get_default_prompt_service,
)
...
class TestPromptService:
    ...
    def test_list_available_prompts(self, temp_prompts_dir):
        ...
        # Check file paths
        assert prompts["test_prompt"] == os.path.join(
            temp_prompts_dir, "test_prompt.txt"
        )
    ...
    def test_unicode_handling(self, temp_prompts_dir):
        ...
        with open(
            os.path.join(temp_prompts_dir, "unicode_prompt.txt"), "w", encoding="utf-8"
        ) as f:
