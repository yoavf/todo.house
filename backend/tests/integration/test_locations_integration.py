...
class TestLocationsIntegration:
    ...
    async def test_location_lifecycle(self, client, test_user_id, db_session):
        ...
        update_data = {
            ...
            "location_metadata": {"floor": 2, "has_window": True, "has_standing_desk": True}
        }
    ...
    async def test_default_locations_persistence(self, client, test_user_id, db_session):
        ...
        kitchen_virtual = next((loc for loc in locations if loc["name"] == "Kitchen"), None)
        ...
        kitchen_saved = next((loc for loc in locations2 if loc["name"] == "Kitchen"), None)
    ...
    async def test_location_with_tasks_relationship(self, client, test_user_id, db_session):
        ...
        garage_tasks = [t for t in tasks if t.get("location") and t["location"]["id"] == location_id]
