...
class TestGetImageEndpoint:
    ...
    async def test_get_image_success(
        ...
        mock_storage.get_public_url.return_value = "https://storage.example.com/images/test-image.jpg"
        ...
        assert result["thumbnail_url"] == "https://storage.example.com/images/test-image.jpg?width=200&height=200&resize=contain&quality=80"
...
class TestImageProxyEndpoint:
    ...
    async def test_proxy_image_success(
        ...
        response = await proxy_image(
            image_id=str(mock_image_record.id),
            session=mock_session
        )
    ...
    async def test_proxy_image_storage_error(
        ...
        # Mock storage error
        mock_storage.download_file = AsyncMock(side_effect=Exception("Storage unavailable"))
    ...
    @pytest.mark.asyncio
    @patch('app.images.storage')
    async def test_proxy_image_invalid_id_format(
