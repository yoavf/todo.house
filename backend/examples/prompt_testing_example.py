...
async def demonstrate_single_prompt_testing():
    ...
    # Display results
    print("\n📊 Test Results:")
    print(f"Status: {'✓ SUCCESS' if result.result_status == TestResult.SUCCESS else '✗ ' + result.result_status.value.upper()}")
...
def demonstrate_test_image_library():
    ...
    custom_description = "Custom maintenance scenario: Dirty garage with oil stains and cluttered tools"
...
async def main():
    ...
    try:
        ...
        successful_tests = sum(1 for r in history if r.result_status == TestResult.SUCCESS)
