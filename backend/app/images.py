...
from .ai.image_processing import ImageProcessingService, ImageProcessingError, ImageValidationError
...
async def analyze_image(
    ...
    generate_tasks: bool = Form(True, description="Whether to generate tasks from analysis"),
    prompt_override: Optional[str] = Form(None, description="Custom prompt for testing")
):
    """
    ...
    **Processing**: Images are compressed for AI analysis to reduce costs
    ...
    if hasattr(image, 'size') and image.size and image.size > config.image.max_image_size_mb * 1024 * 1024:
        raise HTTPException(
    ...
    try:
        ...
        # Process image
        logger.info(f"Starting image analysis for user {user_id}, file: {image.filename}")
