...
class TestUserSettingsIntegration:
    ...
    async def test_get_user_settings_new_user_creation(
        self, client: AsyncClient, test_user_id: str, auth_headers: dict
    ):
        """Test that new users are created automatically on first auth."""
        response = await client.get("/api/user-settings/me", headers=auth_headers)
    ...
    async def test_update_user_settings_invalid_locale(
        ...
        response = await client.patch(
            "/api/user-settings/me", json=update_data, headers=auth_headers
        )
...
class TestLocaleAwareTasksIntegration:
    ...
    async def test_tasks_fallback_to_header_when_no_preference(
        ...
        response_en = await client.get(
            "/api/tasks/", headers={**auth_headers, "Accept-Language": "en-US,en;q=0.9"}
        )
