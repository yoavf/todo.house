...
jest.mock('react-native-paper', () => ({
  ...
  FAB: {
    Group: jest.fn(({ visible, open, onStateChange, actions, onPress, testID }) => {
      ...
      return (
        <MockFABGroup testID={testID || 'fab-group'}>
          ...
          {open && actions?.map((action: any, index: number) => (
            <MockFABGroup
              key={index}
...
jest.mock('@gorhom/bottom-sheet', () => {
  ...
  return {
    BottomSheetModal: React.forwardRef(({ children, onChange, onDismiss }: any, ref: any) => {
