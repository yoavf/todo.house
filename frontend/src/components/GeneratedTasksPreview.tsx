...
import { type ImageAnalysisResponse, type TaskCreate, tasksAPI } from "@/lib/api";
...
export function GeneratedTasksPreview({ 
	...
	const handleSelectAll = () => {
		if (selectedTasks.size === analysisResponse.tasks.length) {
			...
			setSelectedTasks(new Set(analysisResponse.tasks.map((_, index) => index)));
		}
	};
		...
		<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
			<div className="flex justify-between items-start mb-4">
				<div>
					...
					<p className="text-sm text-gray-600 mt-1">
						Found {analysisResponse.tasks.length} potential tasks • 
						Processed in {analysisResponse.processing_time.toFixed(2)}s • 
						Provider: {analysisResponse.provider_used}
					</p>
				</div>
				{onClose && (
					<button
			...
			{analysisResponse.analysis_summary && (
				<div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
					...
					<p className="text-sm text-blue-800">{analysisResponse.analysis_summary}</p>
				</div>
			)}
			...
			<div className="flex justify-between items-center mb-4">
				...
				>
					{selectedTasks.size === analysisResponse.tasks.length ? 'Deselect All' : 'Select All'}
				</button>
						...
						<div className="flex items-start gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<div className="flex items-center gap-4 text-xs text-gray-500">
									<span className={`font-medium ${getConfidenceColor(task.confidence_score)}`}>
										Confidence: {(task.confidence_score * 100).toFixed(0)}%
									</span>
								</div>
							</div>
			...
			<div className="flex gap-3">
				...
				>
					{isCreating ? 'Creating Tasks...' : `Create ${selectedTasks.size} Task${selectedTasks.size !== 1 ? 's' : ''}`}
				</button>
				{onClose && (
					<button
						type="button"
