...
def rate_limit_provider():
    ...
    return MockAIProvider(
        should_fail=True,
        fail_with=AIProviderRateLimitError("Rate limited", retry_after=1),
    )
...
class TestImageProcessingServiceOrchestration:
    ...
    @pytest.mark.asyncio
    async def test_successful_analysis_with_provider(
        self, service_with_provider, sample_image_data
    ):
        """Test successful image analysis with AI provider."""
        result = await service_with_provider.analyze_image_and_generate_tasks(
            image_data=sample_image_data, user_id="test-user"
        )
    ...
    @pytest.mark.asyncio
    async def test_analysis_without_provider(
        self, service_without_provider, sample_image_data
    ):
        ...
        assert (
            result["analysis_summary"] == "Image processed but no AI analysis performed"
        )
        assert result["tasks"] == []
    ...
    @pytest.mark.asyncio
    async def test_analysis_with_generate_tasks_false(
        self, service_with_provider, sample_image_data
    ):
        """Test image analysis with generate_tasks=False."""
        result = await service_with_provider.analyze_image_and_generate_tasks(
            image_data=sample_image_data, user_id="test-user", generate_tasks=False
        )
    ...
    @pytest.mark.asyncio
    async def test_analysis_with_custom_prompt(
        self, service_with_provider, sample_image_data
    ):
        ...
        with patch.object(
            service_with_provider.ai_provider, "analyze_image", new_callable=AsyncMock
        ) as mock_analyze:
    ...
    async def test_invalid_image_raises_validation_error(self, service_with_provider):
        ...
        with pytest.raises(ImageValidationError):
            await service_with_provider.analyze_image_and_generate_tasks(
                image_data=invalid_data, user_id="test-user"
            )
...
class TestRetryLogic:
    ...
    async def test_retry_on_rate_limit_error(
        self, rate_limit_provider, sample_image_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_no_retry_on_general_provider_error(
        self, failing_ai_provider, sample_image_data
    ):
...
class TestPromptGeneration:
    ...
    def test_generate_prompt_with_full_context(self, service_without_provider):
        ...
        context = {"room_type": "kitchen", "focus_areas": ["appliances", "countertops"]}
        prompt = service_without_provider.generate_prompt(context)
...
class TestResponseValidation:
    ...
    def test_tasks_not_list(self, service_without_provider):
        ...
        response = {"analysis_summary": "Summary", "tasks": "not a list"}
        errors = service_without_provider.validate_ai_response(response)
...
class TestErrorHandling:
    ...
    async def test_unexpected_error_wrapping(self, sample_image_data):
        ...
        with patch.object(
            service.preprocessor, "validate_and_preprocess"
        ) as mock_preprocess:
