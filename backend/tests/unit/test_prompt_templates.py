...
class TestHomeMaintenancePromptTemplate:
    ...
    def setup_method(self):
        ...
        self.template_content = """You are a home maintenance expert analyzing an image to identify maintenance tasks.
...
If you cannot identify any maintenance tasks, provide an empty tasks array with an explanation in the analysis_summary."""
    ...
    def test_task_requirements_specification(self):
        ...
        for field in required_fields:
            assert field.lower() in self.template_content.lower(), f"Missing required field: {field}"
    ...
    def test_priority_levels_defined(self):
        ...
        for level in priority_levels:
            assert level in self.template_content, f"Priority level '{level}' not found in template"
    ...
    def test_task_types_completeness(self):
        ...
        for task_type in expected_task_types:
            assert task_type in self.template_content, f"Task type '{task_type}' not found in template"
    ...
    def test_task_type_descriptions_present(self):
        ...
        for pattern, description in task_type_patterns:
            assert re.search(pattern, self.template_content, re.IGNORECASE), f"Missing {description}"
    ...
    def test_confidence_score_ranges_defined(self):
        ...
        for range_text in confidence_ranges:
            assert range_text in self.template_content, f"Confidence range '{range_text}' not found"
    ...
    def test_confidence_score_descriptions(self):
        ...
        for description, context in confidence_descriptions:
            assert description in self.template_content, f"Confidence description '{description}' not found"
            assert context in self.template_content, f"Confidence context '{context}' not found"
    ...
    def test_title_length_constraint(self):
        ...
        assert "max 50 characters" in self.template_content, "Title length constraint not specified"
    
    def test_reasoning_field_requirement(self):
        """Test that reasoning field requirement is present."""
        assert "reasoning field" in self.template_content, "Reasoning field requirement not found"
        assert "explaining why it was identified" in self.template_content, "Reasoning explanation not found"
    
    def test_empty_tasks_handling(self):
        """Test that instructions for handling no maintenance tasks are present."""
        assert "empty tasks array" in self.template_content, "Empty tasks array instruction not found"
        assert "analysis_summary" in self.template_content, "Analysis summary field not mentioned"
    ...
    def test_template_word_count_reasonable(self):
        ...
        assert 100 < word_count < 500, f"Template word count ({word_count}) may be too short or too long"
    ...
    def test_template_readability(self):
        ...
        assert len(numbered_items) >= 5, "Template should use numbered lists for organization"
    ...
    def test_category_examples_provided(self):
        ...
        for category in category_examples:
            assert category in self.template_content, f"Category example '{category}' not found"
    ...
    def test_template_language_professional(self):
        ...
        for word in casual_words:
            assert word not in self.template_content.lower(), f"Casual language '{word}' found in template"
        ...
        for phrase in imperative_phrases:
            assert phrase in self.template_content, f"Imperative phrase '{phrase}' not found"
    ...
    def test_task_field_ordering_logical(self):
        ...
        assert title_pos < description_pos, "Title should be specified before description"
        # Description should come before priority
        assert description_pos < priority_pos, "Description should be specified before priority"
        # Priority should come before category
        assert priority_pos < category_pos, "Priority should be specified before category"
    ...
    def test_confidence_score_boundary_values(self):
        ...
        assert "0.0 to 1.0" in self.template_content, "Confidence score range not specified"
    ...
    def test_all_task_types_have_colon_descriptions(self):
        ...
        for task_type_line in task_type_lines:
            assert task_type_line in self.template_content, f"Task type line '{task_type_line}' not found"
...
If you cannot identify any maintenance tasks, provide an empty tasks array with an explanation in the analysis_summary."""
        ...
        assert self._validate_template_structure(complete_template) == True
    ...
    def test_validate_incomplete_template(self):
        ...
        assert self._validate_template_structure(incomplete_template) == False
...
2. Include a detailed description explaining what needs to be done and why"""
        ...
        assert self._validate_template_structure(template_missing_types) == False
...
3. Assign a priority level (high, medium, low) based on urgency and safety"""
        ...
        assert self._validate_template_structure(template_no_confidence) == False
...
2. Include a detailed description explaining what needs to be done and why"""
        ...
        assert self._validate_template_structure(template_no_priority) == False
...
class TestPromptTemplateIntegration:
    ...
    def test_template_all_priority_levels(self):
        ...
        responses_by_priority = {
            "high": {"tasks": [{"priority": "high", "title": "Emergency repair", "confidence": 0.9}]},
            "medium": {"tasks": [{"priority": "medium", "title": "Routine maintenance", "confidence": 0.7}]},
            "low": {"tasks": [{"priority": "low", "title": "Cosmetic improvement", "confidence": 0.3}]}
        }
...
If you cannot identify any maintenance tasks, provide an empty tasks array with an explanation in the analysis_summary."""
        ...
        assert execution_time < 1.0, f"Template parsing too slow: {execution_time} seconds"
...
Analyze this image and identify specific, actionable home maintenance tasks based on what you observe."""
        ...
        for word in complex_words:
            assert word not in template.lower(), f"Complex word '{word}' found in template"
        
        # Should use active voice
        passive_indicators = ["was analyzed", "were identified", "is being"]
        for indicator in passive_indicators:
            assert indicator not in template.lower(), f"Passive voice indicator '{indicator}' found"
