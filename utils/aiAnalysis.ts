...
function createOpenAIProvider() {
  ...
  logger.debug(
    'AIAnalysis',
    '🔧 Creating OpenAI provider with API key present:',
    !!apiKey,
  )
...
- "Sweep floor" in "Kitchen"`,
    ...
    logger.info(
      'AIAnalysis',
      `✅ High confidence task identified: "${result.object.title}" (${result.object.confidence})`,
    )
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        return {
          success: false,
          error:
            'Network error. This might be due to React Native environment. Please check your internet connection.',
        }
      }
      ...
      if (error.name === 'AI_DownloadError') {
        logger.error(
          'AIAnalysis',
          '📱 AI SDK download error - might be React Native compatibility issue',
        )
        return {
          success: false,
          error:
            'AI SDK compatibility issue with React Native. This feature may need alternative implementation.',
        }
      }
    }
