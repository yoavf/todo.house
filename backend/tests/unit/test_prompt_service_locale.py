...
class TestPromptServiceLocale:
    ...
    def temp_prompts_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(os.path.join(temp_dir, "test_prompt.txt"), "w", encoding="utf-8") as f:
                f.write("Fallback prompt content")
            
            # Create English prompt
            with open(os.path.join(temp_dir, "locales", "en", "test_prompt.txt"), "w", encoding="utf-8") as f:
                f.write("English prompt content")
            
            # Create Hebrew prompt
            with open(os.path.join(temp_dir, "locales", "he", "test_prompt.txt"), "w", encoding="utf-8") as f:
                f.write("Hebrew prompt content - תוכן בעברית")
            
            # Create prompt that only exists in English
            with open(os.path.join(temp_dir, "locales", "en", "english_only.txt"), "w", encoding="utf-8") as f:
                f.write("English only prompt")
            
            # Create prompt that only exists as fallback
            with open(os.path.join(temp_dir, "fallback_only.txt"), "w", encoding="utf-8") as f:
    ...
    def test_error_handling_with_file_permissions(self, temp_prompts_dir):
        ...
        restricted_file = os.path.join(temp_prompts_dir, "locales", "he", "restricted.txt")
