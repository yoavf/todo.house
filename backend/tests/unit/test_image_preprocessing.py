...
class TestImagePreprocessor:
    ...
    def create_test_image(
        self, format_type: str = "JPEG", size: tuple = (100, 100), mode: str = "RGB"
    ) -> bytes:
    ...
    async def test_validate_and_preprocess_image_too_large(self):
        ...
        large_image_data = fake_data * (
            15 * 1024 * 1024 // len(fake_data)
        )  # Approximately 15MB
    ...
    def test_compress_for_ai_minimum_quality_limit(self):
        ...
        with patch.object(
            self.preprocessor.config, "max_ai_image_size_kb", 1
        ):  # 1KB limit
            compressed_data = self.preprocessor._compress_for_ai(image)
    ...
    async def test_validate_and_preprocess_with_custom_config(self):
        ...
        with patch.object(self.preprocessor, "config", custom_config):
            ...
            large_image_data = b"fake_image_data" * (
                7 * FAKE_IMAGE_CHUNK_SIZE
            )  # 7MB of data
...
class TestImagePreprocessorIntegration:
    ...
    def create_complex_test_image(
        self, format_type: str = "JPEG", size: tuple = (500, 400)
    ) -> bytes:
