...
class TestLocationEndpoints:
    ...
    async def test_list_locations_includes_defaults(self, client, test_user_id):
    ...
    async def test_create_default_location_marks_as_default(self, client, test_user_id):
    ...
    async def test_invalid_user_id_format(self, client):
        """Test that invalid user ID format returns 400 error."""
        location_data = {"name": "Test Location"}

        response = await client.post(
            "/locations/", json=location_data, headers={"x-user-id": "invalid-uuid"}
        )
    ...
    async def test_update_location_not_found(self, client, test_user_id):
    ...
    async def test_update_location_wrong_user(self, client, test_user_id):
    ...
    async def test_update_location_all_fields(self, client, test_user_id):
    ...
    async def test_delete_location_not_found(self, client, test_user_id):
        """Test deleting non-existent location returns 404."""
        non_existent_id = uuid.uuid4()

        response = await client.delete(
            f"/locations/{non_existent_id}", headers={"x-user-id": str(test_user_id)}
        )

        assert response.status_code == 404
        assert "Location not found" in response.json()["detail"]

    async def test_delete_location_wrong_user(self, client, test_user_id):
        ...
        other_user_id = uuid.uuid4()
        response = await client.delete(
            f"/locations/{location_id}", headers={"x-user-id": str(other_user_id)}
        )

        assert response.status_code == 404
        assert "Location not found" in response.json()["detail"]

    async def test_list_locations_empty_user(self, client):
        """Test listing locations for a user with no locations shows only defaults."""
        new_user_id = uuid.uuid4()

        response = await client.get(
            "/locations/", headers={"x-user-id": str(new_user_id)}
        )
